...
export const appRouter = router({
  ...
  // ============ CHECKOUT (STRIPE) ============
  checkout: router({
    create: protectedProcedure.input(z.object({ listingId: z.number() })).mutation(async ({ ctx, input }) => {
      const listing = await db.getListingById(input.listingId);
      if (!listing) throw new TRPCError({ code: "NOT_FOUND", message: "Listing not found" });
      ...
      const stripe = new Stripe(ENV.stripeSecretKey, { apiVersion: '2026-07-29.dahlia' });
