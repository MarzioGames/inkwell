import { and, desc, eq, sql, or, lte, gte } from "drizzle-orm";
...
export async function listPosts(communitySlug?: string, sort: 'hot' | 'new' | 'top' = 'hot', limit = 20, offset = 0) {
  const db = await getDb();
  if (!db) return [];
  const conditions = [];
  if (communitySlug) {
...
export async function listListings(status: 'active' | 'sold' | 'removed' = 'active', limit = 20, offset = 0) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(listings).where(eq(listings.status, status)).orderBy(desc(listings.createdAt)).limit(limit).offset(offset);
}
...
export async function globalSearch(query: string, limit = 20) {
  ...
  const postsResult = await db.select().from(posts)
    .where(sql`title LIKE ${q} OR content LIKE ${q}`)
    .orderBy(desc(posts.createdAt))
    .limit(limit);

  const listingsResult = await db.select().from(listings)
    .where(and(
